...
Das Objekt `con_obj` wird dann als erstes Argument in `read_matomo_data.R` genutzt. Siehe unten.
...
read_matomo_data(connection = con_obj, apiModule = "Actions", apiAction = "getPageUrls", date = "2020-09-15", period = "month") %>% 
  select(1:4) %>% 
  janitor::clean_names() %>% 
  knitr::kable()
